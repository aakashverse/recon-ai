...
| # | Decision Point | Considered Using AI? | What We Chose Instead / How AI is Applied | Engineering & Financial Rationale |
|---|---|---|---|---|
...
| **2** | **Tier 2: Rule & Tolerance Matching** | **Yes** — LLM prompt asking if a 2% or 10% deduction is valid. | **Deterministic Arithmetic & In-Memory `RuleCache` (`<20ms`)** | Known statutory TDS deductions (Section 194C 2%, 194J 10%) are fixed statutory equations. Executing exact decimal math via deterministic code guarantees 100% precision and zero hallucinations. |
| **3** | **Split Invoice Reconciliation** | **Yes** — LLM combinatorial reasoning to find combinations. | **Deterministic Bounded Subset-Sum Algorithm (Window Size $\le 3$)** | Subset-sum is a classic bounded computational problem. Deterministic exact combinatorial calculation prevents hallucinations and enforces mathematical conservation of money. |
