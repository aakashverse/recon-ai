...
/**
 * Tier 2: Self-Healing Rule Cache (<20ms)
...
export async function matchTier2(bankTxn) {
  ...
  if (!activeRules.length) {
    return { matched: false, durationMs: performance.now() - startTime };
  }
    ...
    if (matchesRule) {
      ...
      for (const invoice of candidateInvoices) {
        ...
        if (Math.abs(calculatedNet - bankAmount) < 0.05) {
          ...
          ).catch((e) => console.warn('[Tier 2] Rule usage count update error:', e.message));
